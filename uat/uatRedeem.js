...
const baseUrl = 'https://uat-apitest.rocket-tech.app';
const storeUrl = 'https://uat-api-store-admin.rocket-tech.app';
...
export default function () {
  group('User Journey redeem', function () {
    ...
    response = http.get('https://api.line.me/liff/v2/apps/2006704626-ApOP3OJY/contextToken', { headers: getHeaders(lineApiHeaders) });
    ...
    // Step 10: New Step: POST Request for Invite Link
    const postPayload = JSON.stringify({});
    response = http.post(`${baseUrl}/api/rewarding/invite-links/friend`, postPayload, { headers: getHeaders(postRequestHeaders) });
    check(response, { 'Step 10: Invite link created': (res) => res.status === 200 });

    // Step 11: New Step: Get News
    response = http.get(`${baseUrl}/api/rewarding/news`, { headers: getHeaders(userProfileHeaders) });
    check(response, { 'Step 11: News loaded': (res) => res.status === 200 });

    // Step 12: New Step: POST Request to Get All Provinces
    ...
    // Step 13: New Step: get Client Point Expire End Year
    ...
    // Step 14: New Step: get Pdpas To Accept
    ...
    // Step 17: New Step: POST Request to Sentry (Error Tracking)
    ...
    // Step 20: New Step: get Pdpas To Accept Again
    response = http.get(`${baseUrl}/api/rewarding/clientpdpas/getPdpasToAccept`, { headers: getHeaders(userProfileHeaders) });
    check(response, { 'Step 20: get Pdpas To Accept': (res) => res.status === 200 });

    // Step 21: New Step: POST Request to Get All Provinces Again
    ...
    // Step 22: New Step: POST Request to Get All District By AmphurId
    ...
    response = http.get(`${baseUrl}/api/rewarding/histories/677def7442004f2b1cf90807?type=REWARD`, { headers: getHeaders(userProfileHeaders) });
